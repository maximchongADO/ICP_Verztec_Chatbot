...
# Now define subdirectories under the data folder
...
verztec_collection = data_dir / "verztec_logo"
...
def is_verztec_logo(image_path, threshold=5):
    ...
    for filename in os.listdir(verztec_collection):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
            compare_path = os.path.join(verztec_collection, filename)
            try:
...
for file_path in word_files:
    ...
    cleaned_filename = clean_filename_for_embedding(filename_without_ext).strip().replace("\n", "")
    image_filename = cleaned_filename
    file_address = cleaned_dir / (cleaned_filename + ".txt")
